...
def main(clump_model_path, video_path, output_path=None, conf_threshold=0.25, jump_threshold=100):
    # 出力パスが指定されていない場合、入力ファイル名に基づいて自動生成
    if output_path is None:
        base_name = os.path.splitext(os.path.basename(video_path))[0]
        output_path = f"output_{base_name}_hybrid.mp4"
    ...
    last_valid_midpoint = None # 追加: 距離判定用に前回の中間点を保存
    frames_since_last_valid = 0
    INTERPOLATION_FRAME_LIMIT = 10 

    print(f"Starting analysis with JUMP_THRESHOLD = {jump_threshold} pixels per frame.")
    ...
    while cap.isOpened():
        ...
        # --- 2. 物体検出 ---
        clump_results = clump_model(frame, verbose=False, conf=conf_threshold)
        weight_clumps = []
        ...
        if lifter_center is not None:
            cv2.circle(frame, tuple(lifter_center), 10, (255, 0, 255), -1)
        ...
        # Debug print
        # print(f"Frame: {int(cap.get(cv2.CAP_PROP_POS_FRAMES))}, Lifter Center: {lifter_center}, Mid Point: {mid_point}")
    ...
    if not y_coords:
        ...
        if len(smoothed_y) == 0:
            print("Warning: Not enough data to smooth trajectory. Using raw data.")
            smoothed_y = y_coords
