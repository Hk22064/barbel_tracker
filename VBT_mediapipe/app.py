"""
==============================================================================
VBT (Velocity Based Training) Analyzer Prototype
==============================================================================
...
3. スクリプトの実行 (Run Script):
   ...
   - 動画ファイルを使用する場合 (Video File):
     python app.py --video data/squat.mp4

【操作方法 / Controls】
   - 'c': キャリブレーション (Calibrate). 左右の手首の距離を81cmとして計測します。
          (Assume distance between wrists is 81cm)
   - 'q': 終了 (Quit)
...
def main():
    parser = argparse.ArgumentParser(description="VBT Analyzer")
    ...
    while True:
        ...
        if 'left_wrist' in landmarks_dict and 'right_wrist' in landmarks_dict:
            ...
            # Calculate Velocity (Needed for both calibration check and rep logic)
            # Note: Velocity calculation requires calibration factor for m/s,
            # but for "stillness" check we can look at pixel velocity or just assume
            # we need to be stable. 
            # Actually, `calculate_velocity` returns 0 if no calibration factor.
            # We need a raw pixel velocity or just rely on the variance of position 
            # inside `attempt_auto_calibration`?
            # `attempt_auto_calibration` takes `current_velocity_abs`...
            # Let's pass a dummy velocity (0) or calculate raw pixel delta for robustness?
            # For simplicity, let's assume if the USER is standing still, pixel changes are small.
            # `attempt_auto_calibration` logic checks if wrist distance is stable.
            # We can also check bar_y stability here if we wanted.
            
            # Key Handling - MOVED TO END
        ...
        if vbt_analyzer.calibration_factor:
            ...
            if args.video:
                 cal_text = "Calibration: OK (Video Mode)"
        else:
            cal_color = (0, 255, 255)
            if args.video:
        ...
        cv2.imshow('VBT Analyzer Prototype', display_frame)
        
        # Check if window is closed by user (X button)
        if cv2.getWindowProperty('VBT Analyzer Prototype', cv2.WND_PROP_VISIBLE) < 1:
            break
    ...
    filename = f"vbt_report_{timestamp}.txt"
